...
Tabbit 是一只住在浏览器侧边栏的虚拟宠物。它把用户当前的标签页压力转化为看得见的角色状态，并通过轻量任务、庆祝反馈和收藏奖励，帮助用户更愉快地整理浏览空间。
...
**每周有意义互动用户数（WMIU）**：一周内至少完成 2 次不同类型的有意义互动，包括完成专注、主动整理、查看状态或领取成长反馈。
...
| 专注 `focused` | 用户主动开启计时 | 覆盖负载状态、降低打扰 |
| 庆祝 `celebrating` | 完成整理或专注 | 形成即时正反馈 |
...
用户故事：我希望让宠物陪我专注一小段时间，并在结束时获得反馈。

需求：

- 提供 15/25/45 分钟选项；
- 开始后切换到专注状态，显示本地倒计时；
- Service Worker 休眠后通过目标时间恢复，不依赖 `setInterval` 作为真实时钟；
- 用户可暂停或结束；完整完成奖励 2 XP、1 片树叶，每天最多 4 次；
- 通知默认关闭，开启后仅在计时完成时提醒。

验收：休眠、重启或系统时钟小幅变化后误差不超过 5 秒；重复消息不会发放两次奖励。
...
- 展示长时间未访问、重复 URL、同域过多三个候选集合；
- 默认不勾选任何标签；固定、正在播放、当前活动标签默认锁定；
- 操作前显示“将关闭 N 个标签”及清单；
...
- AI 聊天、网页正文摘要和网站分类；
- 用户系统、云后端、排行榜、社交关系；
- 自动阻断网站、自动关闭标签、强制 Pomodoro；
- 付费订阅、广告和第三方埋点；
- 网页漂浮宠物或 `<all_urls>` 内容脚本；
- 浏览历史、书签、下载记录读取。
...
专注和庆祝属于显式状态，优先级高于负载映射。睡眠由静默时段和近期交互共同判断。
...
- [ ] 所有 P0 用户故事通过产品验收。
- [ ] 默认权限只有 `storage`、`alarms`、`sidePanel`。
- [ ] 没有 host permission、内容脚本和远程代码。
